...
tmux \
    ...
    split-window -p 80 "script -f -c 'CT_robot-ros /CT_robot/CmdPort:=/cmd_vel; bash' log/CT_robot-ros-`date +"%Y%m%d-%H%M%S"`.log" \; \
    split-window -p 75 "script -f -c \"eltclsh ; ./end.sh\" log/eltclsh-`date +"%Y%m%d-%H%M%S"`.log" \; \
